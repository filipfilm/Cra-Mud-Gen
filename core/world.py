...
class World:
    ...
    def __init__(self, context_manager=None, llm=None, fallback_mode=False):
        ...
        self.spatial_navigator = SpatialNavigator()  # New logical navigation system
        self.dungeon_layout = []
    ...
    def generate_room(self, room_id: str, theme: str, from_room: str = None, direction: str = None) -> Room:
        ...
        # Use spatial navigator for logical connections
        connections = self._generate_spatial_connections(room_id, from_room, direction, depth)
    ...
    def _generate_spatial_connections(self, room_id: str, from_room: str, direction: str, depth: int) -> List[str]:
        """
        Generate spatial connections using the new spatial navigator system
        """
        # First, ensure the current room exists in the spatial navigator
        if from_room and from_room in self.spatial_navigator.room_positions:
            # Generate this room based on spatial movement from the source
            if room_id not in self.spatial_navigator.room_positions:
                # This room is being generated by moving in 'direction' from 'from_room'
                generated_room = self.spatial_navigator.generate_connected_room(from_room, direction, depth)
                # The room ID might be different from what we expected, use the generated one
                if generated_room != room_id:
                    # Update our tracking - the room system will use the generated ID
                    room_id = generated_room
        else:
            # Add room to spatial navigator if it's not there yet
            if room_id not in self.spatial_navigator.room_positions:
                # Calculate reasonable position for orphaned rooms
                x, y, z = 0, 0, 0
                if room_id != "start_room":
                    # Try to parse position from room name or use random
                    if "_" in room_id:
                        parts = room_id.split("_")
                        if len(parts) >= 2 and parts[1].isdigit():
                            ...
                            if "north" in parts[0]:
                                y = offset
                            elif "south" in parts[0]:
                                y = -offset
                            elif "east" in parts[0]:
                                x = offset
                            elif "west" in parts[0]:
                                x = -offset
                
                self.spatial_navigator.add_room(room_id, x, y, z)
        
        # Generate logical exits for this room
        max_exits = 3 if depth < 5 else 2  # Fewer exits deeper in dungeon
        new_directions = self.spatial_navigator.generate_logical_exits(
            room_id, max_exits, direction
        )
        
        # Create new connected rooms for each direction
        for new_dir in new_directions:
            self.spatial_navigator.generate_connected_room(room_id, new_dir, depth + 1)
        
        # Return connections in the format expected by the room system
        return self.spatial_navigator.get_connection_list(room_id)
    
    def _generate_dynamic_exits_old(self, room_id: str, depth: int, came_from_direction: str, from_room_id: str = None) -> List[str]:
        """
